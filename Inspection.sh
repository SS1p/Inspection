...
check_dependencies() {
    ...
    for dep in "${dependencies[@]}"; do
        if ! command -v $dep &> /dev/null; then
            echo -e "${YELLOW}检测到缺少依赖: $dep，正在尝试安装...${NC}"
...
# 初始化巡检报告
REPORT_FILE="${1:-system_inspection_$(date +%Y%m%d_%H%M%S).log}"
...
for file in "${env_files[@]}"; do
    ...
    if [ -f "$expanded_file" ]; then
        ...
        if [ "$perms" -gt 644 ]; then
            print_error "    不安全权限: $expanded_file (权限$perms，建议≤644)"
...
if (( $(echo "$usage_rate > 80" | bc -l) )); then
    print_error "    警告: 系统句柄使用率超过80%，可能面临耗尽风险"
    add_to_report "    ${RED}警告: 系统句柄使用率超过80%，可能面临耗尽风险${NC}"
fi

# 3.2 进程句柄TOP分析
print_info "  句柄使用TOP 10进程..."
echo "    排名  PID    句柄数  进程名"
echo "    -------------------------"
...
if (( $(echo "$cpu_usage > 80" | bc -l) )); then
    print_error "    警告: CPU使用率超过80%"
    add_to_report "    ${RED}警告: CPU使用率超过80%${NC}"
fi

print_info "    CPU占用TOP5进程:"
...
if (( $(echo "$mem_used_percent > 85" | bc -l) )); then
    print_error "    警告: 内存使用率超过85%"
    add_to_report "    ${RED}警告: 内存使用率超过85%${NC}"
fi

print_info "    内存占用TOP5进程:"
...
df -h | grep -vE 'tmpfs|loop|udev' | awk 'NR>1 {print $0}' | while read line; do
    ...
    if [ $usage -gt 85 ]; then
        print_error "    警告: $(echo $line | awk '{print $6}') 分区使用率超过85%"
        add_to_report "    ${RED}警告: $(echo $line | awk '{print $6}') 分区使用率超过85%${NC}"
    fi
done

print_info "    磁盘I/O性能(5秒采样):"
...
if [ -z "$empty_passwords" ]; then
    ...
    print_error "    发现以下空密码账户: $empty_passwords"
...
if [ -z "$privileged_users" ]; then
    print_info "    未发现除root外的特权用户"
    add_to_report "    ${GREEN}未发现除root外的特权用户${NC}"
else
    print_error "    发现以下非root特权用户: $privileged_users"
        ...
        case $file in
            "/etc/shadow")
                if [ "$perms" -ne 400 ]; then
                    print_error "    不安全的权限: $file 权限为 $perms (应设置为400)"
                    add_to_report "    ${RED}不安全的权限: $file 权限为 $perms (应设置为400)${NC}"
                else
                    print_info "    安全的权限: $file 权限为 $perms"
                    add_to_report "    ${GREEN}安全的权限: $file 权限为 $perms${NC}"
                fi
                ;;
            "/etc/sudoers")
                if [ "$perms" -ne 440 ]; then
                    print_error "    不安全的权限: $file 权限为 $perms (应设置为440)"
            ...
            *)
                if [ "$perms" -gt 644 ]; then
                    print_error "    不安全的权限: $file 权限为 $perms (建议不高于644)"
...
if [ -n "$error_logs" ]; then
    print_warning "    发现错误日志记录:"
    echo "$error_logs" | while read line; do
        echo "      $line"
        add_to_report "      $line"
    done
else
    print_info "    未发现明显错误日志"
...
if [ -n "$failed_logins" ]; then
    print_warning "    发现登录失败记录:"
    echo "$failed_logins" | while read line; do
        echo "      $line"
        add_to_report "      $line"
    done
else
    print_info "    未发现登录失败记录"
...
if [ -n "$suspicious_crons" ]; then
    print_warning "    发现可能存在风险的定时任务:"
    echo "$suspicious_crons" | while read line; do
        echo "      $line"
        add_to_report "      $line"
    done
else
    print_info "    未发现明显风险的系统级定时任务"
